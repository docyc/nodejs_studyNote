 ...
 */
;(function($){
	$.fn.byslider = function(options) {
		var defaults = {
			wraper_id:'wraper',
			tabsCon_id:'tabsCon',
			imagestype:'',               //图片的类型，用于调用侧边栏-自己增加参数-index
			//tabsListcolor:'#00caff',
			...
			Scroll:true,                //是否内容滚动，提供true,false
...
/************************************************/
		...
		//var opts_css = $.extend(defaultCSS,opts_css);
		//alert(opts_css.tabsWraper);
		...
		return this.each(function(i,t) {
			var $this = $(this),
				...
				$tabsCon = $("#"+options.tabsCon_id,$this),
			    $tabsList = $("#"+options.tabsList_id,$this),
				$tabs_img_num = $tabsCon.find("div"),
				$tabsLi = $tabsList.find("li"),
				nTabsLiLen = $tabs_img_num.length, 
				
				$tabsConDiv = $tabsCon.children();
				//alert($tabsCon.attr("id"));
				//*****************************************************//
				//tabsListcolor
				//$tabsLi.css("background-color",options.tabsListcolor);
				if(options.imagestype == "index")
				{
					var images_h = $tabsCon.find("img").height();
					var cebianlan_h = images_h;
					var tabs_h = $tabsList.height();
					//var linum = new array;
					var linum = nTabsLiLen;
					var li_h = $tabsList.find("li").height();
					var margin_h = cebianlan_h - (li_h*linum+5*linum) - 2;
					//alert(li_h);
					//alert("li"+$tabsLi.length);
					//alert("asdasdasd");
					//alert(margin_h);
					$tabsList.css("margin-top", margin_h +"px");
				}
				//********************************************************//
			...
			var $wraper = $("#"+options.wraper_id,$this),
				...
				for(var d = 0; d < nTabsLiLen; d++){
					...
					$.ajax({
						...
						success: function(data, textStatus, jqXHR){
							oThisDiv.removeClass("loading");
							$("#"+tabsCon_id,$this).find("div:eq(" + d + ")").html(data);
							return false;
